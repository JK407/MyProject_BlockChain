...
func (bc *Blockchain) ValidChain(chain []*Block) bool {
	...
	for currentIndex < len(chain) {
		...
		if !bc.ValidProof(b.GetNonce(), b.GetPreviousHash(), b.GetTransactions(), MiningDifficult) {
			return false
		}
...
func (bc *Blockchain) AddBlock(block *Block) error {
	...
	// 将新区块添加到链的副本中
	chainCopy = append(chainCopy, block)

	// 验证整个链的有效性
	if !bc.ValidChain(chainCopy) {
		return fmt.Errorf("无效的区块链")
	}
