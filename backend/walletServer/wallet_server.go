...
func (ws *WalletServer) ListTransaction(w http.ResponseWriter, req *http.Request) {
	fmt.Printf("Call ListTransaction  METHOD:%s\n", req.Method)
	switch req.Method {
	case http.MethodPost:
		var data map[string]interface{}
		// 解析JSON数据
		...
		// 获取JSON字段的值
		blockchainAddress := data["blockchain_address"].(string)
		color.Blue("请求查询账户%s的交易列表", blockchainAddress)
		...
		bcsResp, err := http.Post(ws.Gateway()+"/ListTransactions", "application/json", bytes.NewBuffer(jsonData))
		if err != nil {
			log.Printf("ERROR: 发起请求时发生错误：%v", err)
			http.Error(w, "发起请求时发生错误", http.StatusInternalServerError)
			return
		}
		defer bcsResp.Body.Close()

		// 读取响应体
		respBody, err := ioutil.ReadAll(bcsResp.Body)
		if err != nil {
			log.Printf("ERROR: 读取响应体时发生错误：%v", err)
			http.Error(w, "读取响应体时发生错误", http.StatusInternalServerError)
			return
		}
		...
		err = json.Unmarshal(respBody, &transactions)
		if err != nil {
			log.Printf("ERROR: 解析交易数据时发生错误：%v", err)
			http.Error(w, "解析交易数据时发生错误", http.StatusInternalServerError)
			return
		}
		// 构建带有 from、to 和 value 字段的交易列表
		formattedTransactions := make([]map[string]interface{}, len(transactions))
		for i, t := range transactions {
			formattedTransaction := map[string]interface{}{
				"from":  t.SenderBlockchainAddress,
				"to":    t.RecipientBlockchainAddress,
				"value": t.Value,
			}
			formattedTransactions[i] = formattedTransaction
		}
		...
		if err != nil {
			log.Printf("ERROR: 编码JSON时发生错误：%v", err)
			http.Error(w, "编码JSON时发生错误", http.StatusInternalServerError)
			return
		}
...
func (ws *WalletServer) Run() {
	...
	http.HandleFunc("/ListTransaction", ws.ListTransaction)
